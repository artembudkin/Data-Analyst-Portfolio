...
Finally, to ensure that everything was done correctly, check the "Column quality" under "View" ribbon to make sure there are no errors in any of the tables.

### Creating Date Table
The next step is to create a specific date table with a unique entry for each day.  
Under "Modelling" ribbon, press "New table". For the DAX formula, the following query has been used:  
...
This column will be useful in the further steps for calculating PYTD figures.  

### Table relationships
The next step is to establish relationships between tables to form a star schema.  
The following relationships have been established:  
* Product_name_id[Dim_product] - One-to-many - Product_id[Fact_Sales]  
* Account_id[Dim_account] - Many-to-many - Account_id[Fact_sales] - due to one blank value present, it was not possible to establish one-to-many relationship
...
These are all the base measures that were added to the same named folder.  
From here we need to identify how the business is performing relative to the previous year.  
